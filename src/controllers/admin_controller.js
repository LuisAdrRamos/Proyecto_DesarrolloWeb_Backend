...
// Función para limpiar datos del usuario o admin
const limpiarDatos = (dataBDD) => {
    delete dataBDD.password; // Eliminar si existe
    delete dataBDD.token;
    delete dataBDD.confirmEmail;
    delete dataBDD.createdAt;
    delete dataBDD.updatedAt;
    delete dataBDD.__v;
    return dataBDD;
};
...
const perfil = (req, res) => {
    ...
    const adminLimpio = limpiarDatos(req.adminBDD);
    res.status(200).json({ ...adminLimpio, rol: "admin" });
};
...
const actualizarPassword = async (req, res) => { 
    try {
        // Verificar si req.adminBDD existe
        if (!req.adminBDD) {
            return res.status(401).json({ msg: "Acceso no autorizado, administrador no autenticado" });
        }
